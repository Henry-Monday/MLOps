...
def run_data_prep(dest_path: str = "output/", raw_data_path: str = "./TAXI_DATA_FOLDER/", dataset: str = "green"):
    ...
    # df_train = read_dataframe(
    #     os.path.join(f"{dataset}_tripdata_2022-01.parquet")
    # )
    df_val = read_dataframe(
        os.path.join(raw_data_path, f"{dataset}_tripdata_2022-02.parquet")
    )
    # df_val = read_dataframe(
    #     os.path.join(f"{dataset}_tripdata_2022-02.parquet")
    # )
    df_test = read_dataframe(
        os.path.join(raw_data_path, f"{dataset}_tripdata_2022-03.parquet")
    )
    # df_test = read_dataframe(
    #     os.path.join(f"{dataset}_tripdata_2022-03.parquet")
    # )
